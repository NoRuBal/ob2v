...
function AchievementSystem.New()
	...
	achsys.soundEffect = love.audio.newSource("achievements/res/SoundEffect.ogg", "static")
	...
	achsys.introTitle = "Achievements"
	achsys.introDesc = "This Game Has Achievements."
	...
	if achsys.totalPopupsVerticalSpace > love.graphics.getHeight() - 130 then
		achsys.scrollUI = true
		achsys.maxScrollOffset = achsys.totalPopupsVerticalSpace - love.graphics.getHeight() + 190
		achsys.scrollDirection = 0
...
function AchievementSystem:Update()
	...
	if love.keyboard.isDown("down") then
		if self.scrollUI then
			if not(self.scrollOffset == self.maxScrollOffset) then
	...
	if love.keyboard.isDown("up") then
		if self.scrollUI then
			if not(self.scrollOffset == 0) then
				self.scrollOffset = self.scrollOffset - self.scrollDirection
				self.scrollDirection = 1
				self.scrollWaitTime = 0
			end
...
function AchievementSystem:DrawInGame()
	if self.isDrawing then
